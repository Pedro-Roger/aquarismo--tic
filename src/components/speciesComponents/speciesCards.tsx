...
export default function SpeciesCard({ species, onShowDetails }: SpeciesCardProps) {
      ...
      >
        ...
        <div className="p-6 flex-1 flex flex-col justify-between items-center w-full">
          <div className="flex flex-col items-center w-full">
            <h3 className="text-2xl text-blue-800 font-extrabold mb-1 drop-shadow-sm text-center">
              {species.name}
            </h3>
            <p className="text-base text-cyan-700 italic mb-2 text-center">
              {species.scientificName}
            </p>
            <p className="text-lg font-bold text-green-700 mb-2 text-center">
              R$ {Number(species.price).toFixed(2)}
            </p>
          ...
          <div className="flex gap-3 mt-4 w-full justify-center">
            <button
              ...
              onClick={() => {
                addToCart({
                  id: species.id,
                  name: species.name,
                  price: Number(species.price),
                });
                toast.success("Peixe adicionado ao carrinho!");
              }}
            >
              Comprar
            </button>
            <button
              ...
              onClick={onShowDetails ? onShowDetails : () => setIsModalOpen(true)}
            >
              Ver detalhes
            </button>
          </div>
